...
async function findAndroidDevice(): Promise<string> {
  ...
  if (!line) {
    throw new Error(
      "No connected Android device/emulator found. Boot one first - see docs/RunningTests.md.",
    );
  }
...
async function findIosDevice(): Promise<string> {
  ...
  if (!match || !match[1]) {
    throw new Error("No booted iOS Simulator found. Boot one first - see docs/RunningTests.md.");
  }
  return match[1];
}
