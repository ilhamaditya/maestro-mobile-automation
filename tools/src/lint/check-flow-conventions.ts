  ...
  walkObjects(commandsDoc, (obj, keyPath) => {
    if ("point" in obj) {
      violations.push({
        ...
        detail: `Found a coordinate-based selector at ${keyPath} - use a stable id/text selector instead (see docs/SelectorStrategy.md).`,
      });
    }
  });
  return violations;
  ...
  walkObjects(commandsDoc, (obj, keyPath) => {
    if ("index" in obj && !("id" in obj) && !("text" in obj)) {
      violations.push({
        ...
        detail: `Found an index-only selector at ${keyPath} - pair "index" with an "id" or "text" selector (see docs/SelectorStrategy.md).`,
      });
    }
  });
  return violations;
...
export function checkFeatureLayerIsSelectorFree(filePath: string, commandsDoc: unknown): Violation[] {
  if (!filePath.split(path.sep).includes("features")) {
    return [];
  }
  ...
  commandsDoc.forEach((step, i) => {
    ...
    if (!keys.every((key) => key === "runFlow")) {
      violations.push({
        file: filePath,
        rule: "feature-layer-selector-free",
        detail: `Step ${i} uses "${keys.join(", ")}" - Layer-1 feature flows may only contain runFlow (see docs/FlowGuide.md).`,
      });
    }
  });
  return violations;
...
export function lintFlowFile(filePath: string): Violation[] {
  ...
  return [
    ...
    ...checkFeatureLayerIsSelectorFree(filePath, parsed.commands),
  ];
}

export function lintAllFlows(workspaceDir: string = MAESTRO_WORKSPACE_DIR): Violation[] {
  const files = fg.sync("flows/**/*.yaml", { cwd: workspaceDir, absolute: true });
