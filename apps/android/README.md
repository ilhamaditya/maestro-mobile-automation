...
**Phase 1 placeholder:** the Wikipedia Android app (`org.wikipedia`),
bundled inside Maestro's own official sample archive.
...
- Fetched and checksummed by `tools/src/apps/fetch-sample-apps.ts` into
...
process, then update:
- `appId` in every `.maestro/flows/**/*.yaml` file (currently `org.wikipedia`)
- `APP_ID_ANDROID` in `config/environments/.env.*`
- The onboarding sub-flow's selectors (`.maestro/flows/sub-flows/complete-onboarding-android.yaml`)
  are specific to Wikipedia's first-run carousel and will not apply to a
  different app
